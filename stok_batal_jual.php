...
$halaman = "stok_batal_jual"; // halaman
...
if ($chmod >= 2 || $_SESSION['jabatan'] == 'admin') {
  ...
  <!-- KONTEN BODY AWAL -->
           ...
           $sql    = "select * from transaksimasuk where nota ='$q' order by no";

           $sum=mysqli_fetch_assoc(mysqli_query($conn,"SELECT SUM(hargaakhir) as sub FROM transaksimasuk WHERE nota='$q'"));
           ...
           <?php $kd=$fill['kode'];
           $brg=mysqli_fetch_assoc(mysqli_query($conn,"SELECT sku FROM barang WHERE kode=$kd"));?>
           <td><?php  echo mysqli_real_escape_string($conn, $brg['sku']); ?></td>
           ...
           <button type="button" class="btn btn-danger btn-xs" onclick="window.location.href='component/delete/delete_item?jenis=<?php echo '2'.'&'; ?>kode=<?php echo $fill['kode'].'&'; ?>jumlah=<?php echo $fill['jumlah'].'&'; ?>nota=<?php echo $q.'&'; ?>no=<?php echo $fill['no'].'&'; ?>forward=<?php echo $forward.'&';?>forwardpage=<?php echo $forwardpage.'&'; ?>harga=<?php echo $fill['harga'].'&'; ?>hargabl=<?php echo $fill['hargabeli'].'&'; ?>chmod=<?php echo $chmod; ?>'">Hapus</button>
           <?php } else {}?>
           </td></tr>
           <?php
           $i++;
